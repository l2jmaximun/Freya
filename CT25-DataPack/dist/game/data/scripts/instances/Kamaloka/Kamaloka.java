...
public class Kamaloka extends Quest
{
	 ...
	 */
	private final synchronized void enterInstance(final L2PcInstance player, final int index)
	{
		int templateId;
		try
		{
	 ...
	 */
	private static final void finishInstance(final InstanceWorld world)
	{
	...
	private final void spawnKama(final KamaWorld world)
	{
		int[] npcs;
		int[][] spawns;
		L2Npc npc;
		...
		if (npcs != null)
		{
			...
			final int shaman = Rnd.get(spawns.length); // random position for shaman
		...
		if (tmpWorld instanceof KamaWorld)
		{
			...
			if (world.firstRoom != null)
				...
				if (world.shaman != 0 && world.shaman == objectId)
				{
					...
					for (final L2Spawn spawn : world.firstRoom)
						if (spawn != null)
							spawn.stopRespawn();
	...
	public Kamaloka(final int questId, final String name, final String descr)
	{
		super(questId, name, descr);
		addFirstTalkId(TELEPORTER);
		addTalkId(TELEPORTER);
		for (final int cap : CAPTAINS)
		{
			addStartNpc(cap);
			addTalkId(cap);
		}
		for (final int[] mob : FIRST_ROOM) // shaman
			if (mob != null)
				if (STEALTH_SHAMAN)
					addKillId(mob[1]);
				else
