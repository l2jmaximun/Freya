...
public class PailakaDevilsLegacy extends Quest
{
	...
	private void attackPlayer(final L2Attackable npc, final L2Playable attacker)
	{
	...
	public final String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		...
		else if (npc.getId() == LEMATAN && event.equalsIgnoreCase("lematan_finish_teleport") && !_isOnShip)
		{
			...
			for (final int[] SPAWN : FOLLOWERS_SPAWNS)
			{
				final L2Npc _follower = addSpawn(FOLLOWERS, SPAWN[0], SPAWN[1], SPAWN[2], SPAWN[3], false, 0, true, player.getInstanceId());
				if (_follower != null)
	...
	public final String onAttack(final L2Npc npc, final L2PcInstance attacker, final int damage, final boolean isPet)
	{
		if (npc.getId() == POWDER_KEG && !npc.isDead())
		{
			...
			if (npc.getKnownList() != null)
				for (final L2Character target : npc.getKnownList().getKnownCharactersInRadius(900))
				{
	...
	public final String onKill(final L2Npc npc, final L2PcInstance player, final boolean isPet)
	{
		...
		switch (npc.getId())
		{
			...
			case LEMATAN:
				...
				{
					for (final L2Npc _follower : _followerslist)
						_follower.deleteMe();
	...
	public PailakaDevilsLegacy(final int questId, final String name, final String descr)
	{
