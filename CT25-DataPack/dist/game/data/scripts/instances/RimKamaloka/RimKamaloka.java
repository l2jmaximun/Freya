...
public class RimKamaloka extends Quest
{
	 ...
	 */
	private static final boolean checkConditions(final L2PcInstance player, final int index)
	{
	 ...
	 */
	protected synchronized final void enterInstance(final L2PcInstance player, final int index)
	{
		int templateId;
		try
		{
	 ...
	 */
	private static final void spawnKama(final RimKamaWorld world)
	{
		int[][] spawnlist;
	...
	private static final void spawnNextMob(final RimKamaWorld world, final L2Npc oldNpc, final int npcId, final L2PcInstance player)
	{
		if (world.isFinished)
			return;
		...
		if (!world.spawnedMobs.isEmpty())
			for (final L2MonsterInstance mob : world.spawnedMobs)
			{
	...
	private synchronized final void rewardPlayer(final RimKamaWorld world, final L2Npc npc)
	{
	...
	class LockTask implements Runnable
	{
		...
		LockTask(final RimKamaWorld world)
		{
			_world = world;
		}
		...
		public void run()
		{
			if (_world != null)
			{
				...
				boolean found = false;
				for (final int objectId : _world.allowed)
				{
					final L2PcInstance player = L2World.getInstance().getPlayer(objectId);
					if (player != null && player.isOnline())
					{
	...
	class FinishTask implements Runnable
	{
		...
		FinishTask(final RimKamaWorld world)
		{
			_world = world;
		}
	...
	class DespawnTask implements Runnable
	{
		...
		DespawnTask(final RimKamaWorld world)
		{
			_world = world;
		}
		...
		public void run()
		{
			if (_world != null && !_world.isFinished && !_world.lastAttack.isEmpty() && !_world.spawnedMobs.isEmpty())
			{
				...
				for (final L2MonsterInstance mob : _world.spawnedMobs)
				{
	...
	@Override
	public final String onFactionCall(final L2Npc npc, final L2Npc caller, final L2PcInstance attacker, final boolean isPet)
	{
	...
	public RimKamaloka(final int questId, final String name, final String descr)
	{
		...
		addTalkId(START_NPC);
		addTalkId(REWARDER);
		for (final int[] list : KANABIONS)
		{
			addFactionCallId(list[0]);
			for (final int mob : list)
			{
				addAttackId(mob);
				addKillId(mob);
			}
