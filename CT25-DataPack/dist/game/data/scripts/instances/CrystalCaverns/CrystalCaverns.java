...
public class CrystalCaverns extends Quest
{
	...
	private class CCWorld extends InstanceWorld
	{
		...
		public CCWorld(final Long time)
		{
			endTime = time;
		}
	}
	...
	private static int[][] STEAM3_SPAWNS =
	{
		...
		/*
		 * {22308, 150794, 152455, -12165, 31613},{22308, 150665, 152383, -12165, 32767}, {22308, 151697, 152621, -12167, 31423},{22309, 151061, 152581, -12165, 6228}, {22309, 150653, 152253, -12132, 31343},{22309, 150628, 152431, -12165, 33022}, {22309, 151620, 152487, -12165, 30114},{22309,
		 * 151672, 152544, -12165, 31846}, {22309, 150488, 152350, -12165, 29072},{22310, 151139, 152238, -12132, 1069}
		 */
	};
	...
	private boolean checkOracleConditions(final L2PcInstance player)
	{
		if (debug)
			return true;
		final L2Party party = player.getParty();
	...
	private boolean checkBaylorConditions(final L2PcInstance player)
	{
		final L2Party party = player.getParty();
		if (party == null)
		{
	...
	private void Throw(final L2Character effector, final L2Character effected)
	{
		...
		if (Config.GEODATA > 0)
		{
			final Location destiny = GeoData.getInstance().moveCheck(effected.getX(), effected.getY(), effected.getZ(), _x, _y, _z, effected.getInstanceId());
			_x = destiny.getX();
			_y = destiny.getY();
		}
	...
	protected void stopAttack(final L2PcInstance player)
	{
	...
	protected void runOracle(final CCWorld world)
	{
		world.status = 0;
	...
	protected void runEmerald(final CCWorld world)
	{
		world.status = 1;
		runFirst(world);
	...
	protected void runCoral(final CCWorld world)
	{
		world.status = 1;
		runHall(world);
	...
	protected void runHall(final CCWorld world)
	{
		world.status = 2;

		for (final int[] spawn : SPAWNS)
		{
			final L2Npc mob = addSpawn(CGMOBS[Rnd.get(CGMOBS.length)], spawn[0], spawn[1], spawn[2], spawn[3], false, 0, false, world.instanceId);
			world.npcList1.put(mob, false);
		}
	}

	protected void runFirst(final CCWorld world)
	{
		world.status = 2;
	...
	protected void runEmeraldSquare(final CCWorld world)
	{
		world.status = 3;

		final Map<L2Npc, Boolean> spawnList = new FastMap<>();
		for (final int[] spawn : EMERALD_SPAWNS)
		{
			final L2Npc mob = addSpawn(spawn[0], spawn[1], spawn[2], spawn[3], spawn[4], false, 0, false, world.instanceId);
			spawnList.put(mob, false);
		}
		world.npcList2.put(0, spawnList);
	}

	protected void runEmeraldRooms(final CCWorld world, final int[][] spawnList, final int room)
	{
		final Map<L2Npc, Boolean> spawned = new FastMap<>();
	...
	protected void runDarnel(final CCWorld world)
	{
		world.status = 9;
	...
	protected void runSteamRooms(final CCWorld world, final int[][] spawnList, final int status)
	{
		world.status = status;
	...
	protected void runSteamOracles(final CCWorld world, final int[][] oracleOrder)
	{
		world.oracles.clear();
	...
	protected boolean checkKillProgress(final int room, final L2Npc mob, final CCWorld world)
	{
	...
	public String onFirstTalk(final L2Npc npc, final L2PcInstance player)
	{
		if (npc.getId() == ORACLE_GUIDE_1)
		{
			...
			if (tmpworld instanceof CCWorld)
			{
				...
				{
					final String htmltext = "32281.htm";
					return htmltext;
				}
			}
	...
	public String onSkillSee(final L2Npc npc, final L2PcInstance caster, final L2Skill skill, final L2Object[] targets, final boolean isPet)
	{
		...
		if (npc.getId() >= 32275 && npc.getId() <= 32277 && skill.getId() != 2360 && skill.getId() != 2369)
		{
			...
			{
				for (final L2Npc oracle : ((CCWorld) tmpworld).oracles.keySet())
					if (oracle != npc)
						oracle.decayMe();
	...
	public String onAttack(final L2Npc npc, final L2PcInstance attacker, final int damage, final boolean isPet, final L2Skill skill)
	{
		if (npc.getId() == TEARS)
		{
			...
			if (tmpworld instanceof CCWorld)
			{
				...
				{
					boolean notAOE = true;
					if (skill != null && (skill.getTargetType() == SkillTargetType.TARGET_AREA || skill.getTargetType() == SkillTargetType.TARGET_FRONT_AREA || skill.getTargetType() == SkillTargetType.TARGET_BEHIND_AREA || skill.getTargetType() == SkillTargetType.TARGET_AURA
						|| skill.getTargetType() == SkillTargetType.TARGET_FRONT_AURA || skill.getTargetType() == SkillTargetType.TARGET_BEHIND_AURA))
						notAOE = false;
					if (notAOE)
					{
						for (final L2Npc copy : world.copys)
							copy.onDecay();
						world.copys.clear();
					}
					return "";
	...
	public String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			{
				runEmerald(world);
				for (final L2Npc oracle : world.oracle)
					oracle.decayMe();
			}
			...
			else if (event.equalsIgnoreCase("BaylorMinions"))
			{
				for (int i = 0; i < 10; i++)
				{
					...
					final L2Npc mob = addSpawn(29104, 153571 + x, 142075 + y, -12737, 0, false, 0, false, world.instanceId);
			...
			else if (event.equalsIgnoreCase("RaidStart"))
			{
				...
				for (final L2PcInstance p : world._raiders)
				{
				...
				world._raidStatus = 0;
				for (final L2Npc mob : world._animationMobs)
					mob.doDie(mob);
			...
			else if (event.equalsIgnoreCase("baylor_alarm"))
			{
				...
				{
					final int[] spawnLoc = ALARMSPAWN[Rnd.get(ALARMSPAWN.length)];
			...
			else if (event.equalsIgnoreCase("autoFood"))
			{
				...
				final List<L2Object> crystals = new FastList<>();
				for (final L2Object object : L2World.getInstance().getVisibleObjects(npc, 300))
					if (object instanceof L2ItemInstance && ((L2ItemInstance) object).getId() == CRYSTALFOOD)
						crystals.add(object);
				int minDist = 300000;
				for (final L2Object crystal : crystals)
				{
					final int dx = npc.getX() - crystal.getX();
					final int dy = npc.getY() - crystal.getY();
			...
			{
				final CrystalGolem cryGolem = world.crystalGolems.get(npc);
				final L2CharPosition newpos = new L2CharPosition(cryGolem.foodItem.getX(), cryGolem.foodItem.getY(), cryGolem.foodItem.getZ(), 0);
	...
	private void giveRewards(final L2PcInstance player, final int instanceId, final int bossCry, final boolean isBaylor)
	{
	...
	public String onKill(final L2Npc npc, final L2PcInstance player, final boolean isPet)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			if (world.status == 2 && world.npcList1.containsKey(npc))
			{
				...
				for (final L2Npc crygolem : world.crystalGolems.keySet())
					startQuestTimer("autoFood", 2000, crygolem, null);
			}
			else if (world.status == 4 && npc.getId() == TEARS)
			{
			...
			else if (world.status == 2 && world.keyKeepers.contains(npc))
			{
				...
				for (final L2Npc gk : world.keyKeepers)
					if (gk != npc)
						gk.decayMe();
			}
			...
			{
				if (npc.getId() == 22416)
					for (final L2Npc oracle : world.oracles.keySet())
				...
				if (checkKillProgress(0, npc, world))
				{
					...
					switch (world.status)
					{
						...
						case 25:
							...
							final L2Npc kechi = addSpawn(KECHI, 154069, 149525, -12158, 51165, false, 0, false, world.instanceId);
			...
			else if (world.status == 9 && npc.getId() == DARNEL || world.status == 26 && npc.getId() == KECHI)
			{
				InstanceManager.getInstance().getInstance(world.instanceId).setDuration(300000);
				int bossCry;
				if (npc.getId() == KECHI)
	...
	public String onTalk(final L2Npc npc, final L2PcInstance player)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			else if (npc.getId() == ORACLE_GUIDE_3)
			{
				...
				final Instance baylorInstance = InstanceManager.getInstance().getInstance(world.instanceId);
			...
			else if (npc.getId() == ORACLE_GUIDE_4 && world.status == 31)
			{
				final teleCoord teleto = new teleCoord();
				teleto.instanceId = npc.getInstanceId();
				teleto.x = 153522;
	...
	@Override
	public String onTrapAction(final L2Trap trap, final L2Character trigger, final TrapAction action)
	{
	...
	public CrystalCaverns(final int questId, final String name, final String descr)
	{
		...
		for (final int npc : skillSee)
			addSkillSeeId(npc);
		for (final int mob : MOBLIST)
			addKillId(mob);
		for (final int mob : CGMOBS)
			addKillId(mob);
		for (final int zones : ZONES)
		{
