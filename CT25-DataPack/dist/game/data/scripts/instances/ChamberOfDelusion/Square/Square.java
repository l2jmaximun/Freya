package instances.ChamberOfDelusion.Square;
...
public class Square extends Quest
{
	...
	private class CDWorld extends InstanceWorld
	{
		...
		public CDWorld(final L2Party party)
		{
			super();
			partyInside = party;
	...
	private static final String qn = "Square";
	...
	public void smallpenalty(final InstanceWorld world)
	{
	...
	private void banishStrangers(final CDWorld world)
	{
		final L2Party party = world.getPartyInside();
		
		final L2Npc manager = world.managers[world.currentRoom - 1];
		...
		for (final L2PcInstance player : manager.getKnownList().getKnownPlayersInRadius(1000))
	...
	private void changeRoom(final CDWorld world)
	{
		final L2Party party = world.getPartyInside();
		...
		final long nextInterval = ROOM_CHANGE_INTERVAL + Rnd.get(ROOM_CHANGE_RANDOM_TIME) * 1000;
	...
	private void enter(final CDWorld world)
	{
		final L2Party party = world.getPartyInside();
		...
		final int newRoom = Rnd.get(ROOM_ENTER_POINTS.length - 1) + 1;
	...
	private void earthQuake(final CDWorld world)
	{
		final L2Party party = world.getPartyInside();
	...
	protected void spawnState(final CDWorld world)
	{
	...
	private void exitInstance(final L2PcInstance player)
	{
		...
		if (st != null)
		{
			final String return_point = st.get("return_point");
	...
	{
		final InstanceWorld tmpworld = InstanceManager.getInstance().getPlayerWorld(attacker);
	...
	public String onKill(final L2Npc npc, final L2PcInstance player, final boolean isPet)
	{
		...
		if (tmpworld != null && tmpworld instanceof CDWorld)
			if (npc.getId() == AENKINEL)
			{
				...
				for (final int[] element : BOX_SPAWN_POINTS)
				{
					final L2MonsterInstance box = (L2MonsterInstance) addSpawn(BOX, element[0], element[1], element[2], element[3], false, 0, false, world.instanceId);
	...
	public Square(final int questId, final String name, final String descr)
	{
