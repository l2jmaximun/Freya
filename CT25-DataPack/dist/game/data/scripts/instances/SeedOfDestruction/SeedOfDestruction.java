...
public class SeedOfDestruction extends Quest
{
	...
	private final TIntObjectHashMap<List<SODSpawn>> _spawnList = new TIntObjectHashMap<>();
	...
	private void load()
	{
		...
		try
		{
			...
			final File file = new File(Config.DATAPACK_ROOT + "/data/spawnZones/seed_of_destruction.xml");
			if (!file.exists())
			{
	...
	private boolean checkConditions(final L2PcInstance player)
	{
		...
		for (final L2PcInstance partyMember : channel.getMembers())
		{
	...
	protected boolean checkKillProgress(final L2Npc mob, final SOD1World world)
	{
	...
	private void spawnFlaggedNPCs(final SOD1World world, final int flag)
	{
	...
	protected boolean spawnState(final SOD1World world)
	{
		if (world.lock.tryLock())
			try
			{
				...
				switch (world.status)
				{
					case 0:
						...
						break;
					case 1:
						final ExShowScreenMessage message1 = new ExShowScreenMessage(1, 0, 5, 0, 1, 0, 0, false, 10000, 1, "The enemies have attacked. Everyone come out and fight!!!! ... Urgh~!");
					...
					case 3:
						...
						return true;
					case 4:
						final ExShowScreenMessage message2 = new ExShowScreenMessage(1, 0, 5, 0, 1, 0, 0, false, 10000, 1, "Obelisk has collapsed. Don't let the enemies jump around wildly anymore!!!!");
					...
					case 7:
						...
						break;
					case 8:
						final ExShowScreenMessage message4 = new ExShowScreenMessage(1, 0, 5, 0, 1, 0, 0, false, 10000, 1, "Come out, warriors. Protect Seed of Destruction");
	...
	protected void spawn(final SOD1World world, final int npcId, final int x, final int y, final int z, final int h, final boolean addToKillTable)
	{
		// traps
		...
		final L2Npc npc = addSpawn(npcId, x, y, z, h, false, 0, false, world.instanceId);
		if (addToKillTable)
	...
	protected void setInstanceTimeRestrictions(final SOD1World world)
	{
	...
	private void sendScreenMessage(final SOD1World world, final ExShowScreenMessage message)
	{
	...
	public String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		...
		if (tmpworld instanceof SOD1World)
		{
			...
			{
				final L2PcInstance target = L2World.getInstance().getPlayer(world.allowed.get(getRandom(world.allowed.size())));
				...
				{
					final L2Attackable mob = (L2Attackable) addSpawn(SPAWN_MOB_IDS[getRandom(SPAWN_MOB_IDS.length)], npc.getSpawn().getLocx(), npc.getSpawn().getLocy(), npc.getSpawn().getLocz(), npc.getSpawn().getHeading(), false, 0, false, world.instanceId);
			...
			{
				final L2DoorInstance tmp = InstanceManager.getInstance().getInstance(npc.getInstanceId()).getDoor(FORTRESS_DOOR);
				if (tmp.getCurrentHp() < tmp.getMaxHp())
				{
					world.deviceSpawnedMobCount = 0;
					spawnFlaggedNPCs(world, 6);
					final ExShowScreenMessage message3 = new ExShowScreenMessage(1, 0, 5, 0, 1, 0, 0, false, 10000, 1, "Enemies are trying to destroy the fortress. Everyone defend the fortress!!!!");
			...
			else if (event.equalsIgnoreCase("BodyGuardThink"))
			{
				...
				if (mostHate != null)
				{
					final double dist = Util.calculateDistance(mostHate.getXdestination(), mostHate.getYdestination(), npc.getSpawn().getLocx(), npc.getSpawn().getLocy());
					if (dist > 900)
		...
		if (tmpworld instanceof SOD1World)
		{
			...
			else if (world.status == 6 && npc.getId() == THRONE_POWERFUL_DEVICE)
			{
				...
				if (npc.getId() == TIAT)
				{
					...
					for (final L2Npc mob : InstanceManager.getInstance().getInstance(world.instanceId).getNpcs())
						mob.deleteMe();
	...
	public SeedOfDestruction(final int questId, final String name, final String descr)
	{
