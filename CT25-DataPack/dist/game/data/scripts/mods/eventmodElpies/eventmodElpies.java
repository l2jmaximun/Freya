...
public class eventmodElpies extends Event
{
	...
	public eventmodElpies(final int questId, final String name, final String descr)
	{
	...
	public boolean eventStart()
	{
		...
		final int[] _spawndata = _spawns[location];

		_elpies_count = 0;

		for (int i = 0; i < _option_howmuch; i++)
		{
			final int x = Rnd.get(_spawndata[0], _spawndata[1]);
			final int y = Rnd.get(_spawndata[2], _spawndata[3]);
