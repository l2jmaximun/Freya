...
public class eventmodRace extends Event
{
	...
	public eventmodRace(final int questId, final String name, final String descr)
	{
	...
	protected void StartRace()
	{
		...
		// Get random Finish
		final int location = Rnd.get(0, _locations.length - 1);
	...
	public boolean eventBypass(final L2PcInstance activeChar, final String bypass)
	{
		if (bypass.startsWith("skill"))
		{
			...
			else
			{
				...
				final L2Skill _sk = SkillTable.getInstance().getInfo(_number, 1);
				if (_sk != null)
				{
					_skill = _number;
	...
	private int isRacing(final L2PcInstance player)
	{
		if (_players.isEmpty())
			return 0;
		if (_players.contains(player))
	...
	private void transformPlayer(final L2PcInstance player)
	{
	...
	private void sendMessage(final L2PcInstance player, final String text)
	{
	...
	private void winRace(final L2PcInstance player)
	{
