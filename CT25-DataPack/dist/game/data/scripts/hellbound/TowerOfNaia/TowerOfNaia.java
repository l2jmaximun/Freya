...
public class TowerOfNaia extends Quest
{
	...
	public TowerOfNaia(final int questId, final String name, final String descr)
	{
		...
		for (final int npcId : TOWER_MONSTERS)
			addKillId(npcId);
	...
	public final String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		...
		// Timer. Depending of _challengeState despans all spawned spores, or spores, reached assembly point
			...
			{
				for (final L2Npc spore : _sporeSpawn)
	...
	public String onKill(final L2Npc npc, final L2PcInstance killer, final boolean isPet)
	{
		...
		else if (Arrays.binarySearch(TOWER_MONSTERS, npcId) >= 0)
		{
			...
			for (final L2ZoneType zone : ZoneManager.getInstance().getZones(npc.getX(), npc.getY(), npc.getZ()))
				if (ZONES.containsValue(zone.getId()))
					for (final int i : ZONES.keySet())
				...
				if (sporeGroup >= 0)
				{
					...
					if (Math.abs(_indexCount[sporeGroup]) < ELEMENT_INDEX_LIMIT && Math.abs(_indexCount[sporeGroup]) > 0 && _indexCount[sporeGroup] % 20 == 0 && getRandom(100) < 50)
					{
						...
						for (final L2Npc spore : _sporeSpawn)
							...
							{
								final NpcSay ns = new NpcSay(spore.getObjectId(), Say2.NPC_ALL, spore.getId(), SPORES_NPCSTRING_ID[getRandom(4)]);
	...
	private int getSporeGroup(final int sporeId)
	{
		int ret;
		switch (sporeId)
		{
	...
	protected void initRoom(final int managerId)
	{
		...
		{
			for (final L2Npc npc : _spawns.get(managerId))
	...
	private int moveTo(final L2Npc npc, final int[] coords)
	{
		int time = 0;
		if (npc != null)
		{
			final double distance = Util.calculateDistance(coords[0], coords[1], coords[2], npc.getX(), npc.getY(), npc.getZ(), true);
	...
	private L2Npc spawnOppositeSpore(final int srcSporeId)
	{
	...
	private void startRoom(final int managerId)
	{
		...
		if (SPAWNS.containsKey(managerId))
		{
			...
			final List<L2Npc> spawned = new FastList<>();
	...
	private void removeForeigners(final int managerId, final L2Party party)
	{
		if (party != null && ZONES.containsKey(managerId) && ZoneManager.getInstance().getZoneById(ZONES.get(managerId)) != null)
		{
			...
			for (final L2PcInstance player : zone.getPlayersInside())
				if (player != null)
				{
	...
	private void removeAllPlayers(final int managerId)
	{
	...
	private class StopRoomTask implements Runnable
	{
		...
		public StopRoomTask(final int managerId)
		{
