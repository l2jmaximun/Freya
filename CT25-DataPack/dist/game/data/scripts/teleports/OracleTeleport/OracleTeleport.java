...
public class OracleTeleport extends Quest
{
	...
	public OracleTeleport(final int questId, final String name, final String descr)
	{
		...
		for (final int teleporters : TELEPORTERS)
		{
			addStartNpc(teleporters);
			addTalkId(teleporters);
		}
		for (final int priests : TEMPLE_PRIEST)
		{
			addStartNpc(priests);
			addTalkId(priests);
		}
		for (final int dawn : TOWN_DAWN)
		{
			addStartNpc(dawn);
			addTalkId(dawn);
		}
		for (final int dusk : TOWN_DUSK)
		{
			addStartNpc(dusk);
			addTalkId(dusk);
		}
	...
	public String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		...
		if (event.equalsIgnoreCase("Return"))
		{
			...
			{
				final int x = RETURN_LOCS[st.getInt("id")][0];
				final int y = RETURN_LOCS[st.getInt("id")][1];
				final int z = RETURN_LOCS[st.getInt("id")][2];
		...
		else if (event.equalsIgnoreCase("zigurratDimensional"))
		{
			...
			int i = 0;
			for (final int zigurrat : TELEPORTERS)
			{
				if (zigurrat == npcId)
					break;
				i++;
	...
	public String onTalk(final L2Npc npc, final L2PcInstance player)
	{
		...
		if (Util.contains(TOWN_DAWN, npcId))
		{
			st.setState(State.STARTED);
			int i = 0;
			for (final int dawn : TELEPORTERS)
			{
				if (dawn == npcId)
					break;
				i++;
		...
		if (Util.contains(TOWN_DUSK, npcId))
		{
			st.setState(State.STARTED);
			int i = 0;
			for (final int dusk : TELEPORTERS)
			{
				if (dusk == npcId)
					break;
				i++;
