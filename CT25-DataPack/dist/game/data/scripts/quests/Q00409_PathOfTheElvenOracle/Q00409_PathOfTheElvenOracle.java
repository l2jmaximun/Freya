			...
			switch (npc.getScriptValue())
			{
				case 0:
				{
					switch (npc.getId())
					{
						...
						{
							npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "The sacred flame is ours!"));
							break;
						}
						...
						case TAMIL:
						{
							npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "As you wish, master!"));
							break;
						}
					}
		...
		if (qs != null && qs.isStarted() && npc.isScriptValue(1) && Util.checkIfInRange(1500, npc, killer, true))
			switch (npc.getId())
			{
				case lIZARDMAN_WARRIOR:
				{
					...
					{
						npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "Arrghh...we shall never.. surrender..."));
