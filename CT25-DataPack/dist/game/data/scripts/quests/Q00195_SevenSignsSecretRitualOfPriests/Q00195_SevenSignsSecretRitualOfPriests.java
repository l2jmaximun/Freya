 ...
 */
package quests.Q00195_SevenSignsSecretRitualOfPriests;
...
public class Q00195_SevenSignsSecretRitualOfPriests extends Quest
{
	...
	public Q00195_SevenSignsSecretRitualOfPriests(final int questId, final String name, final String descr)
	{
	...
	public String onEvent(final String event, final QuestState st)
	{
		...
		{
			final int instID = st.getPlayer().getInstanceId();
			if (instID != 0)
		...
		{
			final InstanceWorld world = InstanceManager.getInstance().getPlayerWorld(st.getPlayer());
			if (world == null)
				return "";
	...
	public String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		if (event.equalsIgnoreCase("return"))
		{
			...
			final L2ZoneType zone = getZoneForCharacter(npc);
			if (zone != null)
				switch (zone.getId())
				{
					case 20500:
		...
		switch (st.getState())
		{
			...
			case State.CREATED:
				if (npcId == CLAUDIA_ATHEBALDT)
					...
					{
						final QuestState qs = st.getPlayer().getQuestState(Q00194_SevenSignsMammonsContract.class.getSimpleName());
						if (qs != null)
							if (qs.isCompleted())
	...
	private void handleReturnMagic(final L2PcInstance player, final L2Npc npc)
	{
		if (!npc.isCastingNow())
		{
			switch (npc.getId())
	...
	private synchronized void enterInstance(final L2PcInstance player)
	{
		...
		final int instanceId = InstanceManager.getInstance().createDynamicInstance("SanctumoftheLordsofDawn.xml");
		world = new InstanceWorld();
		world.instanceId = instanceId;
		world.templateId = INSTANCE_ID;
		InstanceManager.getInstance().addWorld(world);
		_log.info("SanctumoftheLordsofDawn: started instance: " + instanceId + " created by player: " + player.getName());
				...
				case START_DOORS:
					...
					for (final L2PcInstance pc : knows)
						if (pc == player && !door.getOpen())
						{
							...
							for (final L2Npc npc : InstanceManager.getInstance().getInstance(instanceId).getNpcs())
	...
	private void spawnMovingGuards(final int instanceId)
	{
	...
	private int getMoveDelay(final int[] array)
	{
	...
	private class movmentTask implements Runnable
	{
		...
		protected movmentTask(final L2Npc npc, final int x1, final int y1, final int z1, final int x2, final int y2, final int z2, final int moveDelay)
		{
			_npc = npc;
			_x1 = x1;
			_y1 = y1;
		...
		public void run()
		{
			...
			final MoveToLocation move = new MoveToLocation(_npc);
	...
	private class returnTask implements Runnable
	{
		...
		protected returnTask(final L2Npc npc)
		{
			_npc = npc;
		}
	...
	private class handleNpcAiAndMoveTask implements Runnable
	{
		...
		protected handleNpcAiAndMoveTask(final L2PcInstance player)
		{
			_player = player;
		}
		...
		public void run()
		{
			if (_player != null)
			{
				final L2ZoneType zone = getZoneForCharacter(_player);
				if (zone != null)
				{
					for (final L2Character c : zone.getCharactersInside().values())
					{
						...
						if (c instanceof L2MonsterInstance)
							...
							{
								final MoveToLocation move = new MoveToLocation(c);
	...
	private static final class Teleport implements Runnable
	{
		...
		public Teleport(final L2PcInstance player, final int[] cords, final int id)
		{
			_player = player;
			_cords = cords;
			_instanceId = id;
	...
	protected static L2ZoneType getZoneForCharacter(final L2Character character)
	{
		L2ZoneType zone = ZoneManager.getInstance().getZoneById(ZONES[0]);
		for (final int i : ZONES)
		{
	...
	{
		new Q00195_SevenSignsSecretRitualOfPriests(195, Q00195_SevenSignsSecretRitualOfPriests.class.getSimpleName(), "Seven Signs, Secret Ritual Of The Priests");
	}
}
