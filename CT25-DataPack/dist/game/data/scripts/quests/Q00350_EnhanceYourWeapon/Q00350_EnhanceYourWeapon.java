...
import ct25.xtreme.gameserver.model.actor.L2Attackable.AbsorberInfo;
...
public class Q00350_EnhanceYourWeapon extends Quest
{
	...
	private static final class LevelingInfo
	{
		...
		public LevelingInfo(final AbsorbCrystalType absorbCrystalType, final boolean isSkillNeeded, final int chance)
		{
	...
	private static final class SoulCrystal
	{
		...
		public SoulCrystal(final int level, final int itemId, final int leveledItemId)
		{
			_level = level;
			_itemId = itemId;
	...
	public Q00350_EnhanceYourWeapon(final int questId, final String name, final String descr)
	{
		super(questId, name, descr);
		addStartNpc(STARTING_NPCS);
		addTalkId(STARTING_NPCS);
		load();
		for (final int npcId : NPC_LEVELING_INFO.keySet())
		{
			addSkillSeeId(npcId);
			addKillId(npcId);
		}
	...
	private static boolean check(final QuestState st)
	{
	...
	private static void exchangeCrystal(final L2PcInstance player, final L2Attackable mob, final int takeid, final int giveid, final boolean broke)
	{
		...
		if (Item != null)
		{
			...
			// Send system message
			final SystemMessage sms = SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1);
	...
	private static SoulCrystal getSCForPlayer(final L2PcInstance player)
	{
	...
	private static boolean isPartyLevelingMonster(final int npcId)
	{
		for (final LevelingInfo li : NPC_LEVELING_INFO.get(npcId).values())
			if (li.getAbsorbCrystalType() != AbsorbCrystalType.LAST_HIT)
				return true;
		return false;
	}

	private static void levelCrystal(final L2PcInstance player, final SoulCrystal sc, final L2Attackable mob)
	{
	 ...
	 */
	public static void levelSoulCrystals(final L2Attackable mob, final L2PcInstance killer)
	{
		...
		if (isPartyLevelingMonster(mob.getId()) && killer.getParty() != null)
			...
			for (final L2PcInstance pl : killer.getParty().getPartyMembers())
			{
				if (pl == null)
					continue;
		...
		switch (mainlvlInfo.getAbsorbCrystalType())
		{
			case PARTY_ONE_RANDOM:
				...
				{
					final L2PcInstance lucky = killer.getParty().getPartyMembers().get(getRandom(killer.getParty().getMemberCount()));
