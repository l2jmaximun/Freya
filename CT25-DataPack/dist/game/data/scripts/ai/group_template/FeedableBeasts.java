...
public class FeedableBeasts extends L2AttackableAIScript
{
	...
	private static class GrowthCapableMob
	{
		...
		public GrowthCapableMob(final int growthLevel, final int chance)
		{
			_growthLevel = growthLevel;
			_chance = chance;
		}

		public void addMobs(final int spice, final int[][] Mobs)
		{
			_spiceToMob.put(spice, Mobs);
		}

		public Integer getMob(final int spice, final int mobType, final int classType)
		{
		...
		public Integer getRandomMob(final int spice)
		{
			int[][] temp;
	...
	public void spawnNext(final L2Npc npc, final int growthLevel, final L2PcInstance player, final int food)
	{
		...
		if (Util.contains(TAMED_BEASTS, nextNpcId))
		{
			if (player.getTrainedBeasts() != null && !player.getTrainedBeasts().isEmpty())
				for (final L2TamedBeastInstance oldTrained : player.getTrainedBeasts())
					oldTrained.deleteMe();
	...
	public String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		if (event.equalsIgnoreCase("polymorph Mad Cow") && npc != null && player != null)
			if (MAD_COW_POLYMORPH.containsKey(npc.getId()))
			{
				...
				npc.deleteMe();
				// spawn the new mob
				final L2Attackable nextNpc = (L2Attackable) Quest.addSpawn(MAD_COW_POLYMORPH.get(npc.getId()), npc);
