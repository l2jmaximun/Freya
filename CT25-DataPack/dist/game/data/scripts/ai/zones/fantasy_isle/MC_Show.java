...
public class MC_Show extends Quest
{
	...
	private static Map<String, Object[]> talks = new FastMap<>();
	private static Map<String, Object[]> walks = new FastMap<>();

	private void load()
	{
		talks.put("1", new Object[]
		{
			messages[1],
			"2",
			1000
		});
		talks.put("2", new Object[]
		{
			messages[2],
			"3",
			6000
		});
		talks.put("3", new Object[]
		{
			messages[3],
			"4",
			4000
		});
		talks.put("4", new Object[]
		{
			messages[4],
			"5",
			5000
		});
		talks.put("5", new Object[]
		{
			messages[5],
			"6",
			3000
		});
		talks.put("8", new Object[]
		{
			messages[8],
			"9",
			5000
		});
		talks.put("9", new Object[]
		{
			messages[9],
			"10",
			5000
		});
		talks.put("12", new Object[]
		{
			messages[11],
			"13",
			5000
		});
		talks.put("13", new Object[]
		{
			messages[12],
			"14",
			5000
		});
		talks.put("15", new Object[]
		{
			messages[13],
			"16",
			5000
		});
		talks.put("16", new Object[]
		{
			messages[14],
			"17",
			5000
		});
		talks.put("18", new Object[]
		{
			messages[16],
			"19",
			5000
		});
		talks.put("19", new Object[]
		{
			messages[17],
			"20",
			5000
		});
		talks.put("21", new Object[]
		{
			messages[18],
			"22",
			5000
		});
		talks.put("22", new Object[]
		{
			messages[19],
			"23",
			400
		});
		talks.put("25", new Object[]
		{
			messages[20],
			"26",
			5000
		});
		talks.put("26", new Object[]
		{
			messages[21],
			"27",
			5400
	...
	private void autoChat(final L2Npc npc, final int stringId, final int type)
	{
	...
	public String onAdvEvent(final String event, final L2Npc npc, final L2PcInstance player)
	{
		if (event.equalsIgnoreCase("Start"))
		{
			...
			{
				final int stringId = (Integer) talks.get(event)[0];
				final String nextEvent = (String) talks.get(event)[1];
				final int time = (Integer) talks.get(event)[2];
				autoChat(npc, stringId, 1);
				startQuestTimer(nextEvent, time, npc, null);
			}
			else if (walks.containsKey(event))
			{
				final int x = (Integer) walks.get(event)[0];
				final int y = (Integer) walks.get(event)[1];
				final int z = (Integer) walks.get(event)[2];
				final String nextEvent = (String) walks.get(event)[3];
				final int time = (Integer) walks.get(event)[4];
