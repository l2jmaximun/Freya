...
public class BeastFarm extends L2AttackableAIScript
{
	...
	private static class GrowthCapableMob
	{
		...
		public GrowthCapableMob(final int chance, final int growthLevel, final int tameNpcId)
		{
			_chance = chance;
		...
		public void addNpcIdForSkillId(final int skillId, final int npcId)
		{
		...
		public int getLeveledNpcId(final int skillId)
		{
	...
	public BeastFarm(final int questId, final String name, final String descr)
	{
	...
	public void spawnNext(final L2Npc npc, final L2PcInstance player, final int nextNpcId, final int food)
	{
		...
		if (Util.contains(TAMED_BEASTS, nextNpcId))
		{
			...
			final L2TamedBeastInstance nextNpc = new L2TamedBeastInstance(IdFactory.getInstance().getNextId(), template, player, food, npc.getX(), npc.getY(), npc.getZ(), true);

			String name = _TamedBeastsData.keySet().toArray(new String[_TamedBeastsData.keySet().size()])[getRandom(_TamedBeastsData.size())];
			final SkillHolder[] skillList = _TamedBeastsData.get(name);
			switch (nextNpcId)
			{
				case 18869:
			...
			for (final SkillHolder sh : skillList)
		...
		else
		{
			...
			final L2Attackable nextNpc = (L2Attackable) Quest.addSpawn(nextNpcId, npc);
			...
			final StatusUpdate su = new StatusUpdate(nextNpc);
