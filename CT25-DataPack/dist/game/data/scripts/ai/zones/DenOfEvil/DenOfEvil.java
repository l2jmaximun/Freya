...
/**
 ** @author Gnacik Dummy AI for spawns/respawns only for testing.
 */

public class DenOfEvil extends L2AttackableAIScript
{
	// Mobs
	protected static final int[] _eye_ids =
	{
		18812,
		18813,
		18814
	...
	private int getSkillIdByNpcId(final int npcId)
	{
	...
	public DenOfEvil(final int questId, final String name, final String descr)
	{
	...
	{
		for (final int[] _spawn : _eye_spawn)
	...
	private class RespawnNewEye implements Runnable
	{
		...
		public RespawnNewEye(final int x, final int y, final int z, final int h)
		{
			_x = x;
			_y = y;
			_z = z;
	...
	private class KashaDestruction implements Runnable
	{
		...
		public KashaDestruction(final L2EffectZone zone)
		{
			_zone = zone;
		}
		...
		{
			for (final L2Character character : _zone.getCharactersInside().values())
			{
				if (character == null)
					continue;
