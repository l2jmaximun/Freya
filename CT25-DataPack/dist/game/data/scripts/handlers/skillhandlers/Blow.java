	...
	public void useSkill(final L2Character activeChar, final L2Skill skill, final L2Object[] targets)
	{
		...
		for (final L2Character target : (L2Character[]) targets)
		{
			...
			if (!skillIsEvaded && Formulas.calcBlowSuccess(activeChar, target, skill))
			{
				...
				final boolean soul = weapon != null && weapon.getChargedSoulshot() == L2ItemInstance.CHARGED_SOULSHOT && (weapon.getItemType() == L2WeaponType.DAGGER || weapon.getItemType() == L2WeaponType.DUALDAGGER || weapon.getItemType() == L2WeaponType.RAPIER);
				final byte shld = Formulas.calcShldUse(activeChar, target, skill);
			...
			if (skillIsEvaded)
			{
				...
				if (target instanceof L2PcInstance)
				{
					final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.AVOIDED_C1_ATTACK);
