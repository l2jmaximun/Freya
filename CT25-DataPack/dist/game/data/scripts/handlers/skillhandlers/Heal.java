...
public class Heal implements ISkillHandler
{
	...
	public void useSkill(final L2Character activeChar, final L2Skill skill, final L2Object[] targets)
	{
		...
		for (final L2Character target : (L2Character[]) targets)
		{
			...
			if (skill.getSkillType() == L2SkillType.HEAL && !skill.isPotion() && Formulas.calcMCrit(activeChar.getMCriticalHit(target, skill)))
				hp *= 3;
			...
			if (target instanceof L2PcInstance)
				if (skill.getId() == 4051)
				{
					final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.REJUVENATING_HP);
					target.sendPacket(sm);
				}
				else if (activeChar instanceof L2PcInstance && activeChar != target)
				{
					final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.S2_HP_RESTORED_BY_C1);
