...
import ct25.xtreme.gameserver.model.L2TradeList.L2TradeItem;
...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	private String itemListHtml(final L2TradeList tradeList, final int page)
	{
	...
	private void showShop(final L2PcInstance activeChar, final int merchantID)
	{
		...
		{
			final MerchantPriceConfig mpc = ((L2MerchantInstance) activeChar.getTarget()).getMpc();
	...
	private boolean storeTradeList(final int itemID, final long price, final int tradeListID, final int order)
	{
		Connection con = null;
		try
		{
			...
			final PreparedStatement stmt = con.prepareStatement("INSERT INTO `" + table + "`(`item_id`,`price`,`shop_id`,`order`) VALUES (?,?,?,?)");
	...
	@SuppressWarnings("resource")
	private void updateTradeList(final int itemID, final long price, final int tradeListID, final int order)
	{
		Connection con = null;
		try
		{
			...
			if (Config.CUSTOM_MERCHANT_TABLES)
			{
				final PreparedStatement stmt = con.prepareStatement("UPDATE `custom_merchant_buylists` SET `price` = ? WHERE `shop_id` = ? AND `order` = ?");
	...
	private void deleteTradeList(final int tradeListID, final int order)
	{
		...
		try
		{
			...
			if (Config.CUSTOM_MERCHANT_TABLES)
			{
				final PreparedStatement stmt = con.prepareStatement("DELETE FROM `custom_merchant_buylists` WHERE `shop_id` = ? AND `order` = ?");
	...
	@SuppressWarnings("resource")
	private int findOrderTradeList(final int itemID, final long price, final int tradeListID)
	{
		Connection con = null;
		int order = -1;
		try
	...
	private void showNpcProperty(final L2PcInstance activeChar, final L2NpcTemplate npc)
	{
		final NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		final String content = HtmCache.getInstance().getHtm(activeChar.getHtmlPrefix(), "data/html/admin/editnpc.htm");
	...
	private void saveNpcProperty(final L2PcInstance activeChar, final String command)
	{
		final String[] commandSplit = command.split(" ");
		...
		try
		{
			...
			{
				Class.forName("ct25.xtreme.gameserver.model.actor.instance." + value + "Instance");
	...
	private void showNpcDropList(final L2PcInstance activeChar, final int npcId, final int page)
	{
		final L2NpcTemplate npcData = NpcTable.getInstance().getTemplate(npcId);
		if (npcData == null)
		{
	...
	{
		final String replyMSG = StringUtil.concat("<html><title>Add drop data</title><body><table width=\"100%\"><tr><td>Npc</td><td>", npcData.name, " (", Integer.toString(npcData.npcId), ")", "</td></tr><tr><td>Item Id</td><td><edit var=\"itemId\" width=80></td></tr><tr><td>Min count</td><td><edit var=\"min\" width=80></td></tr><tr><td>Max count</td><td><edit var=\"max\" width=80></td></tr><tr><td>Category (sweep=-1)</td><td><edit var=\"category\" width=80></td></tr><tr><td>Chance (0-1000000)</td><td><edit var=\"chance\" width=80></td></tr></table><center><br><button value=\"Add\" action=\"bypass -h admin_add_drop ", Integer.toString(npcData.npcId), " $itemId $category $min $max $chance\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"><button value=\"Back to Droplist\" action=\"bypass -h admin_show_droplist ", Integer.toString(npcData.npcId), "\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>");
	...
	@SuppressWarnings("resource")
	private void updateDropData(final L2PcInstance activeChar, final int npcId, final int itemId, final int min, final int max, final int category, final int chance)
	{
		Connection con = null;
		...
		try
		{
			...
			{
				final PreparedStatement statement = con.prepareStatement("UPDATE `custom_droplist` SET `min`=?, `max`=?, `chance`=? WHERE `mobId`=? AND `itemId`=? AND `category`=?");
	...
	private void addDropData(final L2PcInstance activeChar, final int npcId, final int itemId, final int min, final int max, final int category, final int chance)
	{
		...
		try
		{
			...
			final PreparedStatement statement = con.prepareStatement("INSERT INTO `" + table + "`(`mobId`, `itemId`, `min`, `max`, `category`, `chance`) VALUES(?,?,?,?,?,?)");
	...
	@SuppressWarnings("resource")
	private void deleteDropData(final L2PcInstance activeChar, final int npcId, final int itemId, final int category, final boolean confirmed)
	{
		if (!confirmed)
		{
			final String replyMSG = StringUtil.concat("<html><title>Drop Data Delete</title><body><br>Delete drop data.", "<table width=\"100%\"><tr><td>NPC</td><td>", NpcTable.getInstance().getTemplate(npcId).name, " (", Integer.toString(npcId), ")</td></tr><tr><td>Item ID</td><td>", Integer.toString(itemId), "</td></tr><tr><td>Category</td><td>", Integer.toString(category), "</td></tr></table><center><br><button value=\"Delete\" action=\"bypass -h admin_del_drop ", Integer.toString(npcId), " ", Integer.toString(itemId), " ", Integer.toString(category), " 1\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"><button value=\"Back to Droplist\" action=\"bypass -h admin_show_droplist ", Integer.toString(npcId), "\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>");
		...
		try
		{
			...
			if (Config.CUSTOM_DROPLIST_TABLE)
			{
				final PreparedStatement statement = con.prepareStatement("DELETE FROM `custom_droplist` WHERE `mobId`=? AND `itemId`=? AND `category`=?");
				statement.setInt(1, npcId);
				statement.setInt(2, itemId);
			...
			if (updated == 0)
			{
				final PreparedStatement statement = con.prepareStatement("DELETE FROM `droplist` WHERE `mobId`=? AND `itemId`=? AND `category`=?");
				statement.setInt(1, npcId);
				statement.setInt(2, itemId);
				statement.setInt(3, category);
				updated = statement.executeUpdate();
	...
	private void showNpcSkillList(final L2PcInstance activeChar, final int npcId, int page)
	{
		...
		final StringBuffer replyMSG = new StringBuffer("<html><title>Show NPC Skill List</title><body><center><font color=\"LEVEL\">");
	...
	private void showNpcSkillEdit(final L2PcInstance activeChar, final int npcId, final int skillId)
	{
		try
		{
			final StringBuffer replyMSG = new StringBuffer("<html><title>NPC Skill Edit</title><body>");
	...
	private void updateNpcSkillData(final L2PcInstance activeChar, final int npcId, final int skillId, final int level)
	{
		...
		try
		{
			...
			{
				final PreparedStatement statement2 = con.prepareStatement("UPDATE `custom_npcskills` SET `level`=? WHERE `npcid`=? AND `skillid`=?");
	...
	private void showNpcSkillAdd(final L2PcInstance activeChar, final int npcId)
	{
		final L2NpcTemplate npcData = NpcTable.getInstance().getTemplate(npcId);

		final StringBuffer replyMSG = new StringBuffer("<html><title>NPC Skill Add</title><body><table width=\"100%\"><tr><td>NPC: </td><td>");
		replyMSG.append(npcData.getName());
		replyMSG.append(" (");
		replyMSG.append(npcData.npcId);
	...
	private void addNpcSkillData(final L2PcInstance activeChar, final int npcId, final int skillId, final int level)
	{
		...
		try
		{
			...
			if (Config.CUSTOM_NPC_SKILLS_TABLE)
			{
				final PreparedStatement statement = con.prepareStatement("INSERT INTO `custom_npcskills`(`npcid`, `skillid`, `level`) VALUES(?,?,?)");
				statement.setInt(1, npcId);
	...
	private void deleteNpcSkillData(final L2PcInstance activeChar, final int npcId, final int skillId)
	{
		...
		try
		{
			...
			if (npcId > 0)
			{
				...
				if (Config.CUSTOM_NPC_SKILLS_TABLE)
				{
					final PreparedStatement statement = con.prepareStatement("DELETE FROM `custom_npcskills` WHERE `npcid`=? AND `skillid`=?");
					statement.setInt(1, npcId);
					statement.setInt(2, skillId);
					updated = statement.executeUpdate();
					statement.close();
	...
	private void reloadNpcSkillList(final int npcId)
	{
		...
		try
		{
			...
			// without race
			final PreparedStatement statement = con.prepareStatement("SELECT `skillid`, `level` FROM `npcskills` WHERE `npcid`=? AND `skillid` <> 4416");
			statement.setInt(1, npcId);
			...
			if (Config.CUSTOM_NPC_SKILLS_TABLE)
			{
				final PreparedStatement statement2 = con.prepareStatement("SELECT `skillid`, `level` FROM `npcskills` WHERE `npcid`=? AND `skillid` <> 4416");
				statement2.setInt(1, npcId);
				final ResultSet skillDataList2 = statement2.executeQuery();
