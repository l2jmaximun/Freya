...
public class AdminEvents implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(final String command, final L2PcInstance activeChar)
	{
		...
		if (command.startsWith("admin_event_start"))
			try
			{
				if (_event_name != null)
				{
					final Event _event = (Event) QuestManager.getInstance().getQuest(_event_name);
					if (_event != null)
					{
	...
	private void showMenu(final L2PcInstance activeChar)
	{
		...
		for (final Quest event : QuestManager.getInstance().getAllManagedScripts())
			if (event instanceof Event && event.getName().startsWith("eventmod"))
				StringUtil.append(cList, "<font color=\"LEVEL\">" + event.getName() + ":</font>", "<table width=270><tr>", "<td><button value=\"Start\" action=\"bypass -h admin_event_start_menu " + event.getName()
