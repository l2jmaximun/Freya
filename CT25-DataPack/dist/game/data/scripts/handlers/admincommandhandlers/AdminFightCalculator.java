...
public class AdminFightCalculator implements IAdminCommandHandler
{
	...
	private void handleStart(final String params, final L2PcInstance activeChar)
	{
		...
		if (npc1 != null && npc2 != null)
			replyMSG = StringUtil.concat("<html><title>Selected mobs to fight</title>" + "<body>" + "<table>" + "<tr><td>First</td><td>Second</td></tr>" + "<tr><td>level ", String.valueOf(lvl1), "</td><td>level ", String.valueOf(lvl2), "</td></tr>"
				+ "<tr><td>id ", String.valueOf(npc1.npcId), "</td><td>id ", String.valueOf(npc2.npcId), "</td></tr>" + "<tr><td>", String.valueOf(npc1.name), "</td><td>", npc2.name, "</td></tr>" + "</table>" + "<center><br><br><br>"
					+ "<button value=\"OK\" action=\"bypass -h admin_fight_calculator_show ", String.valueOf(npc1.npcId), " ", String.valueOf(npc2.npcId), "\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "</center>" + "</body></html>");
		else if (lvl1 != 0 && npc1 == null)
		{
			...
			for (final L2NpcTemplate n : npcs)
				StringUtil.append(sb, "<tr><td><a action=\"bypass -h admin_fight_calculator lvl1 ", String.valueOf(lvl1), " lvl2 ", String.valueOf(lvl2), " mid1 ", String.valueOf(n.npcId), " mid2 ", String.valueOf(mid2), "\">", n.name, "</a></td></tr>");
		...
		else if (lvl2 != 0 && npc2 == null)
		{
			...
			for (final L2NpcTemplate n : npcs)
				StringUtil.append(sb, "<tr><td><a action=\"bypass -h admin_fight_calculator lvl1 ", String.valueOf(lvl1), " lvl2 ", String.valueOf(lvl2), " mid1 ", String.valueOf(mid1), " mid2 ", String.valueOf(n.npcId), "\">", String.valueOf(n.name), "</a></td></tr>");
	...
	private void handleShow(String params, final L2PcInstance activeChar)
	{
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			if (_shld1 > 0)
				shld1++;
			final boolean _crit1 = Formulas.calcCrit(npc1.getCriticalHit(npc2, null), npc2);
			if (_crit1)
				crit1++;
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			if (_shld2 > 0)
				shld2++;
			final boolean _crit2 = Formulas.calcCrit(npc2.getCriticalHit(npc1, null), npc1);
			if (_crit2)
				crit2++;
		...
		if (params.length() == 0)
			...
			StringUtil.append(replyMSG, "<tr><td width=140>Parameter</td><td width=70>", ((L2NpcTemplate) npc1.getTemplate()).name, "</td><td width=70>", ((L2NpcTemplate) npc2.getTemplate()).name, "</td></tr>");

		StringUtil.append(replyMSG, "<tr><td>miss</td><td>", String.valueOf(miss1), "%</td><td>", String.valueOf(miss2), "%</td></tr>" + "<tr><td>shld</td><td>", String.valueOf(shld2), "%</td><td>", String.valueOf(shld1), "%</td></tr>"
		...
		if (params.length() == 0)
			...
			StringUtil.append(replyMSG, "<button value=\"Retry\" action=\"bypass -h admin_fight_calculator_show ", String.valueOf(((L2NpcTemplate) npc1.getTemplate()).npcId), " ", String.valueOf(((L2NpcTemplate) npc2.getTemplate()).npcId), "\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
