...
public class L2DoorInstanceAction implements IActionHandler
{
	...
	public boolean action(final L2PcInstance activeChar, final L2Object target, final boolean interact)
	{
		...
		if (activeChar.getTarget() != target)
		{
			...
			if (((L2DoorInstance) target).getCastle() != null && ((L2DoorInstance) target).getCastle().getCastleId() > 0 || ((L2DoorInstance) target).getFort() != null && ((L2DoorInstance) target).getFort().getFortId() > 0 && !((L2DoorInstance) target).getIsCommanderDoor())
			...
			else if (activeChar.getClan() != null && ((L2DoorInstance) target).getFort() != null && activeChar.getClan() == ((L2DoorInstance) target).getFort().getOwnerClan() && ((L2DoorInstance) target).isUnlockable() && !((L2DoorInstance) target).getFort().getSiege().getIsInProgress())
				if (!((L2Character) target).isInsideRadius(activeChar, L2Npc.INTERACTION_DISTANCE, false, false))
					activeChar.getAI().setIntention(CtrlIntention.AI_INTENTION_INTERACT, target);
				else
				{
