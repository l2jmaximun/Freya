...
/**
 * Template for item skills handler Only minimum of checks
 */
public class ItemSkillsTemplate implements IItemHandler
{
	...
	public void useItem(final L2Playable playable, final L2ItemInstance item, final boolean forceUse)
	{
		...
		if (!TvTEvent.onScrollUse(playable.getObjectId()) || !DMEvent.onScrollUse(playable.getObjectId()) || !LMEvent.onScrollUse(playable.getObjectId()))
		{
		...
		if (skills != null)
			for (final SkillHolder skillInfo : skills)
			{
				...
				if (itemSkill != null)
				{
					...
					if (isPet)
					{
						final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.PET_USES_S1);
						...
						switch (skillId)
						{
							...
							case 26026:
								...
								else if (buffId != 2037 && buffId != 26025 && buffId != 2032 && buffId != 26026)
									activeChar.shortBuffStatusUpdate(skillId, skillLvl, itemSkill.getBuffDuration() / 1000);
								break;
						}
