...
import ct25.xtreme.gameserver.model.actor.instance.L2MercenaryManagerInstance;
	...
	public boolean useBypass(final String command, final L2PcInstance activeChar, final L2Character target)
	{
		...
		try
		{
			...
			if (actualCommand.equalsIgnoreCase("Territory"))
			{
				...
				final int castleId = Integer.parseInt(st.nextToken());
				activeChar.sendPacket(new ExShowDominionRegistry(castleId, activeChar));
			}
			else if (!(target instanceof L2MercenaryManagerInstance))
				return false;

			final L2MercenaryManagerInstance mercman = (L2MercenaryManagerInstance) target;
			if (actualCommand.equalsIgnoreCase("TW_Multisell"))
			{
				if (st.countTokens() < 1)
					return false;
				final int territoryItemId = Integer.parseInt(st.nextToken());
			...
			else if (actualCommand.equalsIgnoreCase("TW_Buy_List"))
			{
				...
				final NpcHtmlMessage html = new NpcHtmlMessage(mercman.getObjectId());
			...
			else if (actualCommand.equalsIgnoreCase("TW_Buy"))
			{
				...
				{
					final long playerItemCount = activeChar.getInventory().getItemByItemId(itemId).getCount();
			...
			else if (actualCommand.equalsIgnoreCase("TW_Buy_Elite"))
			{
				...
				{
					final int _castleid = mercman.getCastle().getCastleId();
					if (_castleid > 0)
			...
			else if (actualCommand.equalsIgnoreCase("CalcRewards"))
			{
				...
				final int[] reward = TerritoryWarManager.getInstance().calcReward(activeChar);
				final NpcHtmlMessage html = new NpcHtmlMessage(mercman.getObjectId());
