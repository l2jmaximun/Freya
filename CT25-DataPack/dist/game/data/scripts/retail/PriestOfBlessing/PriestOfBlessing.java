...
public class PriestOfBlessing extends Quest
{
		...
		else if (event.equalsIgnoreCase("buy_hourglass"))
		{
			...
			if (st.getQuestItemsCount(57) >= _price_hourglass)
			{
				...
				if (System.currentTimeMillis() > _reuse_time)
				{
					...
					final int _nevit_hourglass = _hg[Rnd.get(0, _hg.length - 1)];
	...
	private int getHGIndex(final int lvl)
	{
		int index = 0;
		if (lvl < 20)
			index = 0;
	...
	public PriestOfBlessing(final int questId, final String name, final String descr)
	{
